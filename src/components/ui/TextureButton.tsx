...
const innerDivVariants = cva(
  "w-full h-full flex items-center relative justify-center text-muted-foreground",
  {
    variants: {
      variant: {
        primary:
      ...
      >
        <div className={cn(innerDivVariants({ variant, size }))}>
          ...
          <div className="absolute flex inset-0">
            {!loading ? (
              <motion.span className="relative size-full"></motion.span>
            ) : (
              <motion.span className="relative h-full">
                <SpinnerLoader />
              </motion.span>
            )}
          </div>
